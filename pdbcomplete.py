...
def load_nalib(libname):
    ...
    ph = {
        ...
        "all_atoms": [
            ...
            "C5",
            "C3'",
        ],  # for masking (corresponds to "coor")
        ...
        "rmsd_atoms": [
            ...
            "C5",
            "C3'",
        ],  # to compute best RMSD (avoid base-phosphate clashes),
        "max_missing": 4,
    }
    pho5 = {
        ...
        "atoms": [
            l[12:16].strip()
            for l in open(lib.dir + "/5PHO.pdb")
    ...
    sugar = {
        ...
        "atoms": [
            l[12:16].strip()
            for l in open(lib.dir + "/sugar.pdb")
            if l.startswith("ATOM")
        ],
        "max_missing": 2,
    }
    ...
    for nuc in bases:
        ...
        nuclatoms = [
            l[12:16].strip()
            for l in open(lib.dir + "/%s.pdb" % nuc)
            if l.startswith("ATOM")
        ]
        ...
        baseatoms = [
            l[12:16].strip()
            for l in open(lib.dir + "/base%s.pdb" % nuc)
            if l.startswith("ATOM")
        ]
        ...
        ph_indices = [
            anr for anr, a in enumerate(lib.nucl[nuc]["all_atoms"]) if a in phlist
        ]
...
def _apply_matrix_multi(atoms_array, pivots, rotmats, offsets):
    ...
    newcoor = np.einsum(
        "ijk,ikl->ijl", cen, rotmats
    )  # diagonally broadcasted form of cen.dot(rotmats)
...
def rank_library(res, libcoor, fit_atoms, rmsd_atoms, all_atoms):
    ...
    coor = np.array(
        [res.coords[a] for a in all_atoms if a in res.coords and a in fit_atoms]
    )
    ...
    libcoor_rmsd_fitted = _apply_matrix_multi(
        libcoor_rmsd_unfitted, pivots, rotmats, offsets
    )
...
def apply_nalib(pdb, lib, manual, heavy=True):
    ...
    new_at_all = []
    for nr, res in enumerate(pdb):
        if res.resname not in na_resnames:
            continue
        new_at = []
        try:
            while 1:  # keep fixing as long as we can
                ...
                for fixmode in (
                    ...
                    if (
                        any([(m in atoms) for m in missing])
                        and len([a for a in fit_atoms if a in res.coords]) >= 3
                    ):
                        break
                else:
                ...
                libcoor_fitted_sorted = rank_library(
                    res, libcoor, fit_atoms, rmsd_atoms, all_atoms
                )
                ...
                print(
                    "optimize resid %s %s" % (res.resname, res.resid), file=sys.stderr
                )
                for nl, libconf in enumerate(libcoor_fitted_sorted):
                    clashing = check_clashes(
                        nl,
                        libconf,
                        lib_complete_indices,
                        new_at_all,
                ...
                if nl == len(libconf):
                    raise FixError(
                        "all lib conformer clash on resid %s %s"
                        % (res.resname, res.resid)
                    )
                #
                for anr, a in enumerate(atoms):
                    if (
                        anr in lib_complete_indices
                    ):  # add "or 1" to replace all atoms, for debugging
                        x, y, z = libconf[anr]
                        res.coords[a] = x, y, z
                        new_at.append([x, y, z])
                if fixmode == "nucleotide":
                    break
