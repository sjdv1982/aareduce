...
def check_pdbs(unbounds, bounds):
    for ub, b in zip(unbounds, bounds):
        ...
        if len(ubha) != len(bha):
            raise Exception(
                "Different number of atoms: %s: %d, %s: %d"
                % (ub.filename, len(ubha), b.filename, len(bha))
            )
        atomcount = 0
        for rub0, rb0 in zip(ub.residues(), b.residues()):
            ...
            if len(rub) != len(rb):
                raise Exception(
                    "Residue layout differs: %s, residue %s: %d atoms; %s, residue %s: %d atoms"
                    % (
                        ub.filename,
                        rub[0].resid.strip(),
                        len(rub),
            ...
            for aub, ab in zip(rub, rb):
                ...
                if aub.name != ab.name or aub.resname != ab.resname:
                    raise Exception(
                        "Residue layout differs, heavy atom %d: %s, residue %s: %s; %s, residue %s: %s"
                        % (
                            atomcount + 1,
                            ub.filename,
                            rub[0].resid.strip(),
                            aub.resname + " " + aub.name,
                            b.filename,
                            rb[0].resid.strip(),
                            ab.resname + " " + ab.name,
                        )
...
def build_atommask(pdbs, atoms):
    ...
    for pdbnr, pdb in enumerate(pdbs):
        ...
        if not ok:
            raise Exception(
                "Atommask: zero atoms selected for molecule %d" % (pdbnr + 1)
            )
...
def fit(atoms1, atoms2):
    ...
    E0 = numpy.sum(numpy.sum(atoms1 * atoms1, axis=0), axis=0) + numpy.sum(
        numpy.sum(atoms2 * atoms2, axis=0), axis=0
    )
...
def multifit(array_atoms1, atoms2):
    ...
    # Initial residual, see Kabsch.
    E0 = numpy.sum(numpy.sum(array_atoms1 * array_atoms1, axis=1), axis=1) + numpy.sum(
        numpy.sum(atoms2 * atoms2, axis=0), axis=0
    )
